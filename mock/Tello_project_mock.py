...
import numpy as np
import cv2
from code_1 import VoiceController
kp.init()
import time

vc = VoiceController()
vc.listen_background()
...
fmode=False
def gestureControl(gesture,fmode,command):
    ...
    if  kp.getKey("w") or gesture == "up" or command=="up":
        ud = speed1

    elif kp.getKey("s")or gesture == "down" or command=="down" :#s
        ud = -speed1
    
    if kp.getKey("a") or gesture == "turn right" or command=="four":
        yv = -speed1

    elif kp.getKey("d") or gesture == "turn left" or command=="three":
        yv = speed1

    if  kp.getKey("q") or gesture == "stop" or  command=="two":
        print("stop")
        pass # me.land(); time.sleep(3)
    if gesture == "flip":
       fmode = not fmode 
...
def trackFace( info, w, pid, pError,ud):
...
while True:
  ...
  #  img = me.get_frame_read().frame
    ...
    command = vc.get_command()
    vals = gestureControl(gesture1,fmode,command)
  ...
  #      me.send_rc_control(vals[0], vals[1], vals[2], vals[3])
    ...
    if fmode ==True:
        ...
        pError = trackFace( info, w, pid, pError,vals[2])
...
# Draw the rectangle on the image
    ...
    if cv2.waitKey(1) & 0xFF == ord('q') or command=="close":
        break
    gesture1="None"
    command="None"
#me.land()
